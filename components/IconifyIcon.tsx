...
export default function IconifyIcon({
  ...
  useEffect(() => {
    ...
    fetch(`${ICONIFY_API}/${encodeURIComponent(icon)}.svg?color=currentColor`)
      .then(async (resp) => {
        const text = resp.ok ? await resp.text() : "";
        const value =
          resp.ok && text.trimStart().startsWith("<svg") ? text : buildFallback(size);
        iconSvgCache[icon] = value;
        setSvg(value);
      })
      .catch(() => {
        iconSvgCache[icon] = buildFallback(size);
        setSvg(buildFallback(size));
      })
