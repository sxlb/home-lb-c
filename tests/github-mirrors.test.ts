...
describe("候选源清单（官方 + 内置/环境变量 + 自定义）", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "home-lb-mirrors-"));
...
describe("自定义代理持久化", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "home-lb-mirrors-file-"));
    process.env.DATA_DIR = tmp;
  });
...
describe("testProxySources（后台连通性测试）", () => {
  ...
  beforeEach(() => {
    tmp = fs.mkdtempSync(path.join(os.tmpdir(), "home-lb-mirrors-test-"));
    process.env.DATA_DIR = tmp;
    delete process.env.GITHUB_API_MIRRORS;
  });
